...
let (winN, ((winBoard: int[,]), (winMarkers: bool[,]))) =
    let rec loop ns (bs: (int[,]*bool[,]) list) winningBoards =
        match ns with
        | [] -> winningBoards
        | n::ns' ->
            let markedBoards = bs |> List.map (markNumber n)
            let (winners, remainingBoards) =
                markedBoards
                |> List.partition (fun (_,m) -> hasWon m)
            let winningBoards' = winners |> List.map (fun w -> n,w)
            let newWinningBoards = (winningBoards @ winningBoards')
            if List.isEmpty remainingBoards then
                newWinningBoards
            else
                loop ns' remainingBoards newWinningBoards
    match loop input boards [] |> List.rev with
    | x::_ -> x
    | [] -> failwith "Unable to find winner"
